...
def is_in_range(data, x, y):
    ...
    if cor_to_index(x, y) < 0 or cor_to_index(x, y) >= len(data):
        return False
...
def get_content(data, x, y):
    ...
    return None if ret == '/' or ret == '*' else data[y * 6 + x]
...
def walk(data, x, y):
    ...
    while len(q) > 0:
        t = q.popleft()
        current = get_content(data, t[0], t[1])
        data = set_content(data, t[0], t[1], '*')
        for x0, y0 in [[t[0], t[1]-1], [t[0], t[1]+1], [t[0]+1, t[1]], [t[0]-1, t[1]]]:
            dest = get_content(data, x0, y0)
            if (current is not None) and (dest is not None) and abs(int(current)-int(dest)) <= 1:
                q.append([x0, y0])

    return data

def resolve(data):
    return walk(data, 2, 0)
